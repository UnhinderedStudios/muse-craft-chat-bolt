...
export const api = {
  ...
  // Generate artist images using ONLY Imagen 4 for testing
  async generateArtistImagesImagenOnly(prompt: string): Promise<{ images: string[]; enhancedPrompt?: string; debug?: any }> {
    // Generate client-side request ID for tracking
    const clientRequestId = `client_imagen_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    // Add automatic prompt prefix for artist generation
    const promptPrefix = "Keep composition, structure, lighting and character position identical, character must be entirely different to the reference image, in other words not a single thing must resemble from the character in the image, this should be erased, pose must be entirely different but very cool to the current image, it should be clear that the person is a music artist. No objects such as guitars, mics, chairs or anything else at all can be present in the image. Character must be entirely replaced with: ";
    const fullPrompt = promptPrefix + prompt;
    
    console.log(`🎨 [${clientRequestId}] Calling Imagen 4 ONLY with enhanced prompt:`, fullPrompt);
    console.log(`🔄 [${clientRequestId}] Using fixed reference image from /reference-frame.png`);
    ...
    try {
      ...
      console.log(`📤 [${clientRequestId}] Sending FormData with fixed reference image (Imagen only):`, {
        type: referenceFile.type,
        size: referenceFile.size,
        name: referenceFile.name
      });
      
      // Use FormData for image upload with imagen_only mode
      const formData = new FormData();
      formData.append('prompt', fullPrompt);
      formData.append('image', referenceFile);
      formData.append('mode', 'imagen_only'); // Special flag for Imagen 4 only
      ...
      if (!apiResponse.ok) {
        const errorData = await apiResponse.json().catch(() => ({}));
        console.error(`❌ [${clientRequestId}] Imagen 4 generation failed:`, errorData);
        throw new Error(errorData.error || "Failed to generate images with Imagen 4");
      }
      
      const data = await apiResponse.json();
      console.log(`✅ [${clientRequestId}] Imagen 4 generation successful:`, data);
      ...
      console.error(`❌ [${clientRequestId}] Error with Imagen 4 generation:`, error);
      throw error;
    }
  },
};
