...
async function generateBaseImageWithGemini(prompt: string, backgroundHex?: string, characterCount?: number, stagingImageBase64?: string, stagingImageMimeType?: string, requestId?: string): Promise<string> {
  ...
  console.log(`🔍 [${requestId}] Full Gemini response:`, JSON.stringify(geminiData, null, 2));
  ...
  console.error(`❌ [${requestId}] No image data found in response:`, JSON.stringify(geminiData, null, 2));
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!magicApiResponse.ok) {
      ...
      console.error(`❌ [${requestId}] MagicAPI /run error:`, { 
