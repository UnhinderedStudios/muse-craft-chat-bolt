...
async function generateWithImagen4(character: string, requestId: string): Promise<string[]> {
  ...
  // Use Generative Language API (same as generate-album-cover function)
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-flash:generateImages?key=${geminiKey}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const data = await response.json();
  console.log(`📥 [${requestId}] Imagen 4 response:`, JSON.stringify(data, null, 2));

  if (!data.candidates || data.candidates.length === 0) {
    console.error(`❌ [${requestId}] No candidates in Imagen 4 response`);
    throw new Error("No images generated by Imagen 4");
  }

  const images: string[] = [];
  for (const candidate of data.candidates) {
    if (candidate.image?.jpegBytes) {
      const base64Image = candidate.image.jpegBytes;
