...
async function generateWithImagen4(character: string, requestId: string): Promise<string[]> {
  const geminiKey = Deno.env.get("GEMINI_API_KEY");
  if (!geminiKey) {
    throw new Error("GEMINI_API_KEY not found");
  }

  console.log(`🎨 [${requestId}] Calling Imagen 4 with character: "${character}"`);
  
  // Simplify prompt for Imagen - remove complex constraints, just focus on the character
  const simplifiedPrompt = `A music artist or performer: ${character}. Professional portrait style, high quality, artistic lighting.`;
  
  console.log(`🎯 [${requestId}] Imagen 4 prompt: "${simplifiedPrompt}"`);

  // Use the correct Imagen API format (same as generate-album-cover function)
  const response = await fetch(`https://aiplatform.googleapis.com/v1/projects/disco-entry-314600/locations/us-central1/publishers/google/models/imagen-4.0-generate-001:predict`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    ...
    console.error(`❌ [${requestId}] Imagen 4 API error:`, errorText);
    throw new Error(`Imagen 4 API error: ${response.status} - ${errorText}`);
  }

  const data = await response.json();
  console.log(`📥 [${requestId}] Imagen 4 response structure:`, Object.keys(data));

  if (!data.predictions || data.predictions.length === 0) {
    console.error(`❌ [${requestId}] No predictions in Imagen 4 response`);
  ...
  for (const prediction of data.predictions) {
    if (prediction.bytesBase64Encoded) {
      ...
      console.log(`✅ [${requestId}] Imagen 4 image added, size: ${base64Image.length} chars`);
    }
  }

  if (images.length === 0) {
    console.error(`❌ [${requestId}] No valid images in predictions:`, data.predictions);
    throw new Error("No valid images extracted from Imagen 4 response");
  }

  console.log(`🎉 [${requestId}] Imagen 4 generated ${images.length} image(s) successfully`);
  return images;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!images.length) {
      ...
      console.log(`🔄 [${requestId}] Trying Imagen 4 as fallback...`);
      
      try {
        const imagenImages = await generateWithImagen4(CURRENT_CHARACTER, requestId);
        if (imagenImages && imagenImages.length > 0) {
          console.log(`✅ [${requestId}] Imagen 4 fallback successful! Generated ${imagenImages.length} image(s)`);
          
          return new Response(
            JSON.stringify({
              images: imagenImages,
              enhancedPrompt: CURRENT_CHARACTER,
              fallbackUsed: "imagen4",
              requestId,
              debug: {
        ...
        console.error(`❌ [${requestId}] Imagen 4 fallback also failed:`, imagenError);
      }
      
      return new Response(
        JSON.stringify({ 
          error: "No images generated by Gemini 2.5 Flash Image Preview or Imagen 4 fallback after multiple attempts",
          raw: generationJson,
          requestId
        }),
