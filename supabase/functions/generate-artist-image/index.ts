...
async function generateWithImagen4(character: string, requestId: string): Promise<string[]> {
  ...
  // Create a simplified prompt for Imagen 4
  ...
  // Use exact same format as working generate-album-cover function
  ...
  console.log(`📡 [${requestId}] Full request to Google:`, JSON.stringify(requestBody, null, 2));
  ...
  if (!response.ok) {
    ...
    console.error(`❌ [${requestId}] Imagen 4 API error: ${response.status} - ${errorText}`);
    throw new Error(`Imagen 4 API error: ${response.status} - ${errorText}`);
  }

  const json = await response.json();
  console.log(`📥 [${requestId}] Imagen 4 response:`, JSON.stringify(json, null, 2));
  ...
  if (!images.length) {
    console.error(`❌ [${requestId}] No images in Imagen 4 response:`, json);
    throw new Error("No images generated by Imagen 4");
  }

  console.log(`🎉 [${requestId}] Imagen 4 success: ${images.length} images generated`);
  return images;
}
